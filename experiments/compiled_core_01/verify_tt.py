...
from tt_search import history_context, negamax, pack_mate, search, unpack_mate
...
def main():
    ...
    result = {"positions": rows, "context_and_mate_checks": True,
              "sha256": {name: hashlib.sha256((root / name).read_bytes()).hexdigest()
                         for name in ("compiled_search.py", "tt_search.py", "verify_tt.py")}}
    (root / "tt_verification.json").write_text(json.dumps(result, indent=2))

...
def evaluate(board: chess.Board, params: EvalParameters | None = None) -> int:
    ...
    if params is None and _FAST_EVAL is not None:
        # Fast path: bit-identical numba kernel for the active parameter set.
        try:
            return _FAST_EVAL(board)
        except Exception:  # never let the fast path lose a game
            pass
...
# numba fast path
# ---------------------------------------------------------------------------
# `evaluate_white_relative` walks python-chess objects and dominates leaf cost.
# `fast_eval` is a numba kernel that reproduces it exactly on raw bitboards
# (see the parity probe in scratch/). It is optional: if numba is missing, or
# anything goes wrong, the reference path above is used unchanged.
_FAST_EVAL: Callable[[chess.Board], int] | None = None
FAST_EVAL_READY: bool = False


def warm_fast_eval() -> bool:
    """Compile and warm the numba evaluation kernel. Safe to call twice."""
    global _FAST_EVAL, FAST_EVAL_READY
    if FAST_EVAL_READY:
        return _FAST_EVAL is not None
    FAST_EVAL_READY = True
    if os.environ.get("MILKYWAY_FAST_EVAL", "1") == "0":
        return False
    try:
        from fast_eval import build_fast_eval

        fn = build_fast_eval(ACTIVE_PARAMS)
        if fn is None:
            return False
        # Force compilation inside the init budget, not on the clock: run the
        # kernel on a real position and check it against the reference.
        probe = chess.Board("r1bqkb1r/pppp1ppp/2n2n2/4p3/2B1P3/5N2/PPPP1PPP/RNBQK2R w KQkq - 4 4")
        if fn(probe) != evaluate_white_relative(probe):
            return False
        _FAST_EVAL = fn
        return True
    except Exception:
...
def fast_eval_active() -> bool:
    """True when the numba evaluation kernel is in use."""
    return _FAST_EVAL is not None

...
def main() -> None:
    output = HERE / "confirmation_bank_02.json"
    partial = HERE / "confirmation_bank_generation_02.jsonl"
    assert not output.exists() and not partial.exists(), "Preserve existing generation evidence"
    protocol_path = HERE / "CONFIRMATION_BANK_PROTOCOL_02.json"
    protocol = json.loads(protocol_path.read_text())
    index = HERE / "confirmation_exclusions_02.sqlite"

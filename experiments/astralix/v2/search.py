        ...
        for index, move in enumerate(ordered):
            ...
            try:
                if index == 0:
                    ...
                    # Checking moves are never reduced: a quiet check is the
                    # most forcing quiet move (consistent with the futility
                    # exemption above). SAFETY: gives_check() pushes
                    # internally and raises on non-pseudo-legal input, in
                    # which case its finally-pop steals a caller-pushed move
                    # and corrupts the board (crashed 12/12 unguarded on
                    # 2026-09-07). The is_pseudo_legal guard below is pure
                    # bit ops and makes that unreachable: gives_check only
                    # ever sees legal moves, so its push/pop always balances.
                    reduced = depth - 1
                    do_lmr = (
                        not self._emergency
                        ...
                        and not in_check
                        and not (
                            board.is_pseudo_legal(move) and board.gives_check(move)
                        )
                    )
                    if do_lmr:
                        reduced = depth - 2

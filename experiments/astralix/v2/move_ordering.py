...
def order_root_moves(
    ...
    for move in moves:
        if tt_move is not None and move == tt_move:
            ...
            # Guarded: gives_check() raises on non-pseudo-legal moves and its
            # internal finally-pop would then steal a caller-pushed move,
            # corrupting the board. All inputs here are legal, but the guard
            # makes the theft primitive unreachable by construction.
            gives_check = board.is_pseudo_legal(move) and board.gives_check(move)

...
checking moves are extended one ply at depth>=1 nodes; see has_non_pawn_material.
    ...
    for i in range(count):
        ...
        if i == 0 or depth <= 0:
            value, _ = negamax(board, -side, undo[7], undo[8], child_halfmove,
                               child_base, -beta, -alpha, ply + 1, history,
                               history_count + 1, stats, deadline, coefficient, 0,
                               tt_keys, tt_context, tt_data, killers, move_history, lmr_enabled,
                               True, ext_used + ext)
        else:
            reduced_depth = child_base
            ...
            if reduce and not stats[1] and -value > alpha:
                stats[4] += 1
                value, _ = negamax(board, -side, undo[7], undo[8], child_halfmove,
                                   child_base, -alpha - 1, -alpha, ply + 1, history,
                                   history_count + 1, stats, deadline, coefficient, 0,
                                   tt_keys, tt_context, tt_data, killers, move_history, lmr_enabled,
                                True, ext_used + ext)

...
if _HAVE_NUMBA:
    ...
    @njit(cache=False)
    def _slider_attacks(sq, occ, rays, dirs):
        ...
        for i in range(dirs.shape[0]):
            ...
            if blockers:
                if d <= 3:
                    first = np.int64(_lsb_index(blockers))
                else:
                    first = np.int64(_msb_index(blockers))
    ...
    def _eval_kernel(
        ...
        while t:
            ...
            if (ubp & passer_masks[sq]) == 0:
                ...
                if pawn_att[64 + sq] & uwp:
                    wpmg += P[24]
                    wpeg += P[25]
                elif knight_att[sq] & uwn:
                    wpmg += P[24]
                    wpeg += P[25]
                elif king_att[sq] & uwk:
                    wpmg += P[24]
                    wpeg += P[25]
                elif _slider_attacks(sq, occ, rays, bishop_dirs) & (uwb | uwq):
                    wpmg += P[24]
                    wpeg += P[25]
                elif _slider_attacks(sq, occ, rays, rook_dirs) & (uwr | uwq):
                    wpmg += P[24]
                    wpeg += P[25]
        ...
        while t:
            ...
            if (uwp & passer_masks[64 + sq]) == 0:
                ...
                if pawn_att[sq] & ubp:
                    bpmg += P[24]
                    bpeg += P[25]
                elif knight_att[sq] & ubn:
                    bpmg += P[24]
                    bpeg += P[25]
                elif king_att[sq] & ubk:
                    bpmg += P[24]
                    bpeg += P[25]
                elif _slider_attacks(sq, occ, rays, bishop_dirs) & (ubb | ubq):
                    bpmg += P[24]
                    bpeg += P[25]
                elif _slider_attacks(sq, occ, rays, rook_dirs) & (ubr | ubq):
                    bpmg += P[24]
                    bpeg += P[25]
                ...
                for df in (-1, 0, 1):
                    for dr in (-1, 0, 1):
                        ...
                        if 0 <= f < 8 and 0 <= r < 8:
                            tsq = (r << 3) | f
                            attacked = False
                            if pawn_att[64 + tsq] & ubp:
                                attacked = True
                            elif knight_att[tsq] & ubn:
                                attacked = True
                            elif king_att[tsq] & ubk:
                                attacked = True
                            elif _slider_attacks(tsq, occ, rays, bishop_dirs) & (ubb | ubq):
                                attacked = True
                            elif _slider_attacks(tsq, occ, rays, rook_dirs) & (ubr | ubq):
                                attacked = True
                            if attacked:
                                score += P[37]
            if ubq:
                ...
                for df in (-1, 0, 1):
                    for dr in (-1, 0, 1):
                        ...
                        if 0 <= f < 8 and 0 <= r < 8:
                            ...
                            if pawn_att[tsq] & uwp:
                                attacked = True
                            elif knight_att[tsq] & uwn:
                                attacked = True
                            elif king_att[tsq] & uwk:
                                attacked = True
                            elif _slider_attacks(tsq, occ, rays, bishop_dirs) & (uwb | uwq):
                                attacked = True
                            elif _slider_attacks(tsq, occ, rays, rook_dirs) & (uwr | uwq):
                                attacked = True
                            if attacked:
                                score += P[37]
            if uwq:
...
def build_fast_eval(params: Any) -> Callable[[Any], int] | None:
    ...
    def fast_eval(board: Any) -> int:
        ...
        return _eval_kernel(
            np.uint64(board.pawns & occ_co[True]),
            np.uint64(board.knights & occ_co[True]),
            np.uint64(board.bishops & occ_co[True]),
            np.uint64(board.rooks & occ_co[True]),
            np.uint64(board.queens & occ_co[True]),
            np.uint64(board.kings & occ_co[True]),
            np.uint64(board.pawns & occ_co[False]),
            np.uint64(board.knights & occ_co[False]),
            np.uint64(board.bishops & occ_co[False]),
            np.uint64(board.rooks & occ_co[False]),
            np.uint64(board.queens & occ_co[False]),
            np.uint64(board.kings & occ_co[False]),

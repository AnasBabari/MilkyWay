...
# Minimum soft budget (ms) that must still be available for an "emergency"
# search to be allowed past the depth-4 floor. Below this the clock is genuinely
# nearly gone and the shallow cap is the right call.
EMERGENCY_DEEP_MIN_SOFT_MS: float = 150.0
...
class MilkyWayEngine:
    ...
    def __init__(self) -> None:
        # 4x the old table. Entries are slotted (~100 B), so this stays well
        # inside the 2 GB platform budget while lifting the hit rate.
        self.tt = TranspositionTable(max_entries=524288)
    ...
    def choose_move(self, fen: str, time_left_ms: int) -> str:
        ...
        # The 6 s / depth-4 cliff is too blunt for short arena clocks: on a
        # 10 s base it triggers after ~16 moves, pinning the engine to depth 4
        # for the remaining two thirds of the game while it still finishes with
        # seconds of unspent clock. Gate the cap on the real budget instead --
        # if the allocation is still meaningful, allow the deeper search.
        if budget.emergency and budget.soft_ms >= EMERGENCY_DEEP_MIN_SOFT_MS:

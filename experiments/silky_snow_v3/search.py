...
# Late move pruning: at shallow depths the quiet moves beyond this index are
# vanishingly unlikely to be best, so they are skipped outright (as opposed to
# LMR, which still searches them at reduced depth). Quadratic in depth so the
# pruning gets gentler the nearer we are to the horizon.
LMP_MAX_DEPTH: int = 3
LMP_BASE: int = 4

# Square bitboards, hoisted: indexing a module global is markedly cheaper than
# repeatedly resolving chess.BB_SQUARES through the module attribute.
BB_SQUARES = chess.BB_SQUARES

# Victim values keyed by python-chess piece type. Built once instead of
# materialising a fresh dict literal for every capture in quiescence.
_VICTIM_VALUES: dict[int, int] = {
    chess.PAWN: 100,
    chess.KNIGHT: 320,
    chess.BISHOP: 330,
    chess.ROOK: 500,
...
class Searcher:
    ...
    def _negamax(
        ...
        if self._is_repetition(board, key):
            return DRAW_SCORE
        if self._drawn_by_rule(board):
            return DRAW_SCORE
        ...
        # Hoist the bitboards for a branch-free capture test. board.is_capture()
        # costs a method call plus an is_en_passant() call on every move at
        # every node, which the profile shows as a top-10 hotspot.
        them_bb = board.occupied_co[not board.turn]
        pawns_bb = board.pawns
        ep_sq = board.ep_square
        fut_cut = static + fut_margin

        # Late move pruning. Only when the node is not tactical itself: in
        # check or with a mate score in the window, pruning risks tactics.
        lmp_limit = 0
        if (
            not self._emergency
            ...
            lmp_limit = LMP_BASE + depth * depth
        ...
        for index, move in enumerate(ordered):
            ...
            if (BB_SQUARES[to_sq] & them_bb) or move.promotion is not None:
                tactical = True
            elif (
                ep_sq is not None
                and to_sq == ep_sq
                and (BB_SQUARES[from_sq] & pawns_bb)
                and abs(to_sq - from_sq) in (7, 9)
            ):
                # En passant: victim is a pawn but the target square is empty.
                tactical = True
            else:
                tactical = False
            # Futility: skip late quiet moves that cannot raise alpha. The score
            # test runs first because it is nearly free, whereas proving a move
            # is not a check needs a push/pop. Most moves fail the score test,
            # so this removes the vast majority of those push/pop pairs.
            if futile and index > 0 and not tactical and fut_cut <= alpha:
                board.push(move)
                gives_check_quick = board.is_check()
                board.pop()
                if not gives_check_quick:
                    continue
            # Late move pruning: drop this quiet move entirely.
            if lmp_limit and index >= lmp_limit and not tactical:
                continue
            board.push(move)
            self._stack_keys.append(board._transposition_key())
            try:
    ...
    def _quiescence(self, board: chess.Board, alpha: int, beta: int, ply: int, qply: int) -> int:
        ...
        # _is_repetition() bails out immediately below halfmove_clock 4, so
        # building the 11-tuple transposition key first was pure waste on the
        # overwhelming majority of quiescence nodes (captures reset the clock).
        # Hoisting the guard keeps the behaviour identical and drops the cost.
        ...
        if in_check:
            ...
            best = -INF
            for move in ordered:
                track_child = board.halfmove_clock >= 3
                board.push(move)
                if track_child:
                    self._stack_keys.append(board._transposition_key())
                try:
        ...
        if not captures:
            return alpha
        # Order captures by MVV-LVA (deterministic). Queen promotions precede underpromotions.
        captures.sort(
            key=lambda m: (
                -capture_mvv_lva(board, m),
                (((6 - m.promotion) if m.promotion else 7) << 12)
                | (m.from_square << 6)
                | m.to_square,
            )
        )
        qcap_limit = 4 if self._emergency else len(captures)
        # Hoisted for the en-passant test; captures of an occupied square are
        # resolved from the bitboards, which avoids building a Piece object.
        them_bb = board.occupied_co[not board.turn]
        pawns_bb = board.pawns
        ep_sq = board.ep_square
        for move in captures[:qcap_limit]:
            ...
            if not self._emergency and move.promotion is None:
                to_sq = move.to_square
                if ep_sq is not None and to_sq == ep_sq and (
                    BB_SQUARES[move.from_square] & pawns_bb
                ) and abs(to_sq - move.from_square) in (7, 9):
                    victim_value = 100
                elif BB_SQUARES[to_sq] & them_bb:
                    pt = board.piece_type_at(to_sq)
                    victim_value = _VICTIM_VALUES[pt] if pt is not None else 0
                else:
                    victim_value = 0
                if stand_pat + victim_value + 150 < alpha:
                    continue
            # A child can only be a repetition once four reversible plies have
            # piled up, so only then is its key worth computing. Captures reset
            # the clock, which is most of quiescence.
            track_child = board.halfmove_clock >= 3
            board.push(move)
            if track_child:
                self._stack_keys.append(board._transposition_key())
    ...
    def _drawn_by_rule(self, board: chess.Board) -> bool:
        """Fifty-move / insufficient material, with cheap short-circuits.

        Both checks sit on the hottest path (every quiescence node), so the
        expensive python-chess calls are guarded by bitboard tests that settle
        almost every position: the fifty-move rule needs a halfmove clock of
        100, and any pawn, rook or queen means mating material exists.
        """
    ...
    def _is_repetition(self, board: chess.Board, key: Hashable) -> bool:
        """Two-fold repetition draw within the search, or a known repeat.

        A position can only repeat after at least four reversible plies, so
        when the halfmove clock is below four the whole scan is skipped. That
        removes most of the cost, because captures (which dominate quiescence)
        reset the clock to zero.
        """
        if board.halfmove_clock < 4:
            return False
        rep = self.game_history.get(key, 0)
        if rep >= 2:
            return True

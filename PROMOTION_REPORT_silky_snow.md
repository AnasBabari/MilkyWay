...
| **silky-snow (pure-speed, confirm)** | 50 | 100 | **48.5%** | +41 =15 -44 | -10 | [40%, 58%] | No |

**Interpretation:** The pure-speed version (behaviour-identical, 10% faster) scores **48.5%** on the definitive 50-pair confirm — statistically indistinguishable from 50%. At fixed time control, a faster engine finishes its search sooner and sits idle; it does not search deeper because the time manager budgets the same per move. The 10% speedup does not translate to measurable Elo at this time control.
...
Even with all verified optimizations (numba eval, search cuts, inline ordering), the engine is only ~10% faster than the baseline. At **fixed time control**, that translates to **zero Elo** — the faster engine finishes its search sooner and sits idle. It does not search deeper because the time manager budgets the same soft limit per move. The 10% speedup only matters if the time control itself is increased (e.g., at 120s+0.5s the engine would reach slightly higher depth on average, but so would the opponent).

### 3.3 The Stockfish gap
At 10s+0.1s, the engine is at **-130 Elo** vs Stockfish level 7 (17.5% over 20 games). Closing that to 0 Elo would require a fundamentally stronger evaluation or a 3-5x speedup, neither of which is achievable with micro-optimizations.

### 3.4 Why +70% vs ourselves is impossible here
`flagship_speed_final/agent.zip` is **byte-identical** to our live engine. Scoring 70% requires **+147 Elo** over ourselves. The only way to gain that much is a genuinely better algorithm (e.g., numba movegen, leaf-level NN eval, or months of tuning on a cluster) — not a 10% speedup that doesn't even buy depth at fixed TC.
